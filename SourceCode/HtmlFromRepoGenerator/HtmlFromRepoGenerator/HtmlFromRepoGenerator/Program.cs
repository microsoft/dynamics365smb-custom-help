namespace HtmlFromRepoGenerator
{
    ...
    class Program
    {
        ...
        /// "Incorrect structure of docfx.json"
        /// </summary>
        private const string OpenPublishingRedirectionJsonIncorrect = "Incorrect structure of .openpublishing.redirection.json";
        #endregion
        ...
        private static void RunMain(CommandLineArguments parameters)
        {
            ...
            if (!RepoHelper.TryToCheckClonedRepoOpenPublishingRedirectionJson(parameters.Out, out string pathToOpenPublishingRedirectionJson))
            {
                Logger.LogError($"{Error}: File {pathToOpenPublishingRedirectionJson} does not exist");
                Exit(ExitCodeEnum.InvalidRepo);
            }

            if (!OpenPublishingRedirectionJsonHelper.IsOpenPublishingRedirectionJsonCorrect(pathToOpenPublishingRedirectionJson))
            {
                Logger.LogError(OpenPublishingRedirectionJsonIncorrect);
                Exit(ExitCodeEnum.InvalidRepo);
            }

            Dictionary<string, string> pathRedirectPairs = OpenPublishingRedirectionJsonHelper.GetPathRedirectPairs(pathToOpenPublishingRedirectionJson);
            ...
            foreach (string f in files)
            {
                ...
                LinkProcessorBase proc = new ConceptualLinkProcessor(Logger, repoLocalRoot, parameters.ReplaceUrl, baseEnUsUrl, baseWoExtUrl, externalText, repoLocalRoot != null ? f.Replace(repoLocalRoot, "").TrimStart('\\') : "", repoEnUsLocalRoot, content, newContent, pathRedirectPairs);
                ...
                {
                    ProcessCopiedFiles(repoLocalRoot, parameters.ReplaceUrl, baseEnUsUrl, baseWoExtUrl, externalText, repoEnUsLocalRoot, proc.GetCopiedFiles(), pathRedirectPairs);
                }
            }
        ...
        private static void ProcessCopiedFiles(string baseDir, string baseUrl, string baseEnUsUrl, string baseWoExtUrl, string externalText, string enRepository, ImmutableArray<string> copiedFiles, Dictionary<string, string> pathRedirectPairs)
        {
            foreach (string file in copiedFiles)
            {
                ...
                StringBuilder newContent = new StringBuilder();
                LinkProcessorBase proc = new ConceptualLinkProcessor(Logger, baseDir, baseUrl, baseEnUsUrl, baseWoExtUrl, externalText, file.TrimStart('~').TrimStart('\\').TrimStart('/'), enRepository, content, newContent, pathRedirectPairs);
                bool hasModified = proc.ProcessContentLinks();
                ...
                if (proc.GetCopiedFiles().Length > 0)
                {
                    ProcessCopiedFiles(baseDir, baseUrl, baseEnUsUrl, baseWoExtUrl, externalText, enRepository, proc.GetCopiedFiles(), pathRedirectPairs);
                }
            }
        }
