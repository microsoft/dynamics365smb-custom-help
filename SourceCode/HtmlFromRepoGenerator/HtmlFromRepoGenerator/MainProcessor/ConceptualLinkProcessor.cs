...
namespace MainProcessor
{
    public class ConceptualLinkProcessor : LinkProcessorBase
    {
        ...
        /// The new content
        /// </summary>
        private readonly Dictionary<string, string> _pathRedirectPairs;
        #endregion
        ...
        public ConceptualLinkProcessor(ILogger logger, string baseDir, string baseUrl, string baseEnUsUrl, string baseWoExtUrl, string externalText, string sourceFilePath, string enRepository, string content, StringBuilder newContent, Dictionary<string, string> pathRedirectPairs) : base(logger, baseDir, baseUrl, baseEnUsUrl, baseWoExtUrl, externalText, sourceFilePath, enRepository)
        {
            _content = content;
            _newContent = newContent;
            _newContent.Append(content);
            _pathRedirectPairs = pathRedirectPairs;
        ...
        public override bool ProcessContentLinks()
        {
            ...
            foreach (FoundLink link in links.GroupBy(k => k).Select(k => k.Key))
            {
                ...
                {
                    string href;
                    Uri currentFileUri = new Uri(Path.Combine(BaseDir, SourceFilePath));
                    Uri currentTargetUri = new Uri(Path.GetFullPath(Path.Combine(Path.GetDirectoryName(Path.Combine(BaseDir, SourceFilePath)), linkClear.TrimStart('/'))));
                    Uri baseDirectoryUri = new Uri(BaseDir.EndsWith("\\") ? BaseDir : BaseDir + "\\");
                    string keySourcePath = baseDirectoryUri.MakeRelativeUri(currentTargetUri).OriginalString;
                    if (_pathRedirectPairs.TryGetValue(keySourcePath, out string valueRedirectUrl))
                    {
                        Uri redirectUri = currentFileUri.MakeRelativeUri(new Uri(Path.Combine(Path.GetDirectoryName(currentTargetUri.OriginalString), valueRedirectUrl)));
                        string redirectUriString = redirectUri.OriginalString.Trim();
                        if (!redirectUriString.EndsWith(".md"))
                        {
                            redirectUriString = String.Concat(redirectUriString, ".md");
                        }
                        _newContent.Replace(link.FullMatch, link.FullMatch.Replace(link.Link, redirectUriString));
                        Logger.LogInfo($"Info: Found {keySourcePath} in {fname}, redirecting to {redirectUriString}");
                        HasModified = true;
                        href = BuildFullUrl("/" + SourceFilePath, redirectUriString);
                    }
                    else
                    {
                        href = BuildFullUrl("/" + SourceFilePath, link.Link);
                    }
